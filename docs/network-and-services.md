...
- **Proxmox:** add each VF as a **PCI device** (`hostdev`) on the **VyOS** VM; ensure **IOMMU** grouping allows it (Intel **VT-d** on, **`intel_iommu=on iommu=pt`** on the host where applicable). On the owner’s **legacy Z270** Proxmox host, **`GRUB_CMDLINE_LINUX_DEFAULT`** also needed **PCI realloc / bus-size** tweaks — see **`docs/hardware-requirements.md` § Proxmox host — GRUB (legacy Intel tower, SR-IOV)**.
