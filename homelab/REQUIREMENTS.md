...
- **VyOS is the only client-facing router**, and the only DHCP server. Trusted/Guest/IoT DNS and US-WAN NAT stay on VyOS. India Internet for VLAN 40 is fail-closed through `asus-nuc` (CURRENT: India-GW + Tailscale; PLANNED: VyOS `wg2`). Clients still use `10.10.40.1` and do not run Tailscale.
...
- WireGuard on VyOS (`wireguard/`). WAN IPv6 on `eth1` is already CURRENT. `wg0` is live: `10.10.80.0/24` UDP `51820` = Trusted, first peer `sumit-iphone` at `10.10.80.2/32` (`wg0.md`). It is not in `commands.txt` until exported (omit the private key). `wg1` and `wg2` are not deployed. Not on a Services LXC. Not Tailscale-US. `wg1` `10.10.81.0/24` UDP `51821` = Guest (`wg1.md`); VyOS `wg2` `10.10.82.0/30` UDP `51822` = VLAN 40 → `asus-nuc` (`wg2.md`). The NUC interface stays `wg-india` (this image only allows VyOS names `wgN`). The NUC initiates and never uses `AllowedIPs = 0.0.0.0/0`. VyOS `allowed-ips 0.0.0.0/0` is cryptokey only and must not become the **main** default. Inside the tunnels stays IPv4. Road-warrior DNS is the tunnel gateway. VLAN 40 DNS stays `1.1.1.1` via PBR. No VPS. WAN holes: UDP `51820`–`51822` on `eth1` (IPv4 and IPv6). Default `Endpoint` is a DDNS **A** name so the NUC and phones resolve IPv4. A spare AAAA name is a separate profile if `eth1` has a GUA. No GUA = IPv6 inbound WG unused; IPv4 still works.
- After `wg2` fail-closed tests pass, retire CT 108. Tailscale leaves the US house. Tailscale remains on `asus-nuc` and on travel devices (India exit). It is not a VLAN 40 fallback. Once `wg2` works, VLAN 40 does not use Tailscale; Tailscale on the NUC still has to work when `wg-india` is down.
...
- Tailscale on `asus-nuc` is permanent remote management. The NUC is unattended. Adding VyOS `wg2` / NUC `wg-india` must not flush nftables, edit Tailscale tables, routes, exit-node settings, ACLs, or the Tailscale service, or change the NUC default route. Procedure: `wireguard/wg2.md`.
