...
CT 108 (`10.10.0.5`) is still installed and is not on this path. Do not point table 40 back at it except as a rollback. Known-good VyOS, including `wg2` and rule 400 out `wg2`, is `vyos/commands.txt` (WireGuard `private-key` values are `redacted`). Procedure: `wireguard/wg2.md`. India SSID/VLAN/DHCP: `unifi.md`. The NUC `Endpoint` is `nj.sumitgahoi.me:51822`. VyOS publishes that A record from `eth1` (`vyos/ddns.md`).

**Services** may use the Internet. Services must not initiate connections into Trusted, Guest, IoT, or India. Trusted may initiate into Services; established/related return traffic is allowed. Forward default-drop is the isolation; do not add explicit Services→internal drop rules. UniFi OS Server is on Services (`10.10.0.2`). AdGuard is on Services (`10.10.0.4`, CT 110): DNS only. VyOS stays the DHCP server. Trusted, Guest, and IoT keep DHCP option 6 as the VLAN gateway. AdGuard holds `*.home.arpa` and DoT upstreams. Trusted DHCP `domain-name` is `home.arpa`, so short names on Trusted expand to that zone. IoT may use the Internet out `eth1` only. IoT DNS is its gateway. Guest and IoT may resolve `home.arpa` if they query it; they do not get that search domain. AdGuard down means those three VLANs lose DNS. India DNS stays PBR via `wg2`. CT 108 is still on Services (`10.10.0.5`) and is not the VLAN 40 path. A US Tailscale guest will not be deployed. `10.10.0.3` is unused.
