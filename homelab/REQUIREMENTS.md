...
- **VyOS is the only client-facing router**, and the only DHCP server. Trusted/Guest/IoT DNS and US-WAN NAT stay on VyOS. India Internet for VLAN 40 is fail-closed through `asus-nuc` (CURRENT: VyOS `wg2` / NUC `wg-india`). CT 108 is deprecated and is not the path. Clients still use `10.10.40.1` and do not run Tailscale.
...
**IoT** may use the Internet, and only out `eth1`. IoT must not reach Trusted, Guest, India, Services, or VyOS management. DHCP/DNS to its gateway is allowed. DNS stays the gateway, which forwards to AdGuard. VyOS is not an NTP server.
...
CT 108 (`10.10.0.5`) is deprecated and is not on this path. Stop it; do not destroy it in that sitting (`wireguard/wg2.md` section 7). `10.10.0.5` stays reserved until the container is destroyed. Do not point table 40 at it unless the India egress requirement is changed on purpose (`tailscale-india/`). Known-good VyOS, including `wg2` and rule 400 out `wg2`, is `vyos/commands.txt` (WireGuard `private-key` values are `redacted`). Procedure: `wireguard/wg2.md`. India SSID/VLAN/DHCP: `unifi.md`. The NUC `Endpoint` is `nj.sumitgahoi.me:51822`. VyOS publishes that A record from `eth1` (`vyos/ddns.md`).

**Services** may use the Internet. Services must not initiate connections into Trusted, Guest, IoT, or India. Trusted may initiate into Services; established/related return traffic is allowed. Forward default-drop is the isolation; do not add explicit Services→internal drop rules. UniFi OS Server is on Services (`10.10.0.2`). AdGuard is on Services (`10.10.0.4`, CT 110): DNS only. VyOS stays the DHCP server. Trusted, Guest, and IoT keep DHCP option 6 as the VLAN gateway. AdGuard holds `*.home.arpa` and DoT upstreams. Trusted DHCP `domain-name` is `home.arpa`, so short names on Trusted expand to that zone. IoT may use the Internet out `eth1` only. IoT DNS is its gateway. Guest and IoT may resolve `home.arpa` if they query it; they do not get that search domain. AdGuard down means those three VLANs lose DNS. India DNS stays PBR via `wg2`. CT 108 is deprecated (`10.10.0.5` reserved until destroy) and is not the VLAN 40 path. A US Tailscale guest will not be deployed. `10.10.0.3` is unused.
...
- Stop deprecated CT 108 (`wg2.md` section 7): `onboot 0`, then `pct stop 108`. Do not `tailscale logout`. Do not destroy it in that sitting. Destroy only after VLAN 40 has stayed healthy with the LXC stopped (normal use, and a VyOS or Proxmox reboot). Sections 6 and 6b passed: VLAN 40 stays on `wg2` with `tailscaled` stopped, and Tailscale SSH still works with `wg-india` down. Tailscale leaves the US house. Tailscale remains on `asus-nuc` and on travel devices (India exit). It is not a VLAN 40 fallback.
- Tailscale: not on VyOS. Not on a US LXC while CT 108 is deprecated. Do not build `tailscale-us`.
...
- VLAN 40 via Tailscale again (CT 108). The procedure is `tailscale-india/setup.md`. Doing it replaces `wg2` as the India egress requirement. It is not a second path and not a `wg2` fallback.
